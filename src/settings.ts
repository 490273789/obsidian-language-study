...
function cloneDefaultSettings(): MyPluginSettings {
...
export function normalizeSettings(raw: unknown): MyPluginSettings {
    ...
    if (isRecord(raw.dictionaries)) {
        const rawDictionaries = raw.dictionaries;
        Object.keys(settings.dictionaries).forEach((id) => {
            const dictionary = rawDictionaries[id];
            if (!isRecord(dictionary)) {
                return;
            }
            if (typeof dictionary.enable === "boolean") {
                settings.dictionaries[id].enable = dictionary.enable;
            }
            const priority = normalizePort(
                dictionary.priority,
...
export class SettingTab extends PluginSettingTab {
    ...
    querySettings(containerEl: HTMLElement) {
        ...
        (Object.keys(dicts) as DictionaryId[]).forEach((dict) => {
