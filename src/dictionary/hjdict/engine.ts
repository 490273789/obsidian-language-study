...
import { EMPTY_SAFE_HTML, sanitizeToSafeHtml } from "@/utils/safeHtml";
...
export const search: SearchFunction<HjdictResult> = async (
    ...
    return fetchDirtyDOM(`https://dict.hujiang.com/${langCode}/${encodeURIComponent(text)}`, {
        cookies,
    })
...
function handleDOM(
    ...
    let header: HTMLString = EMPTY_SAFE_HTML;
    ...
    const entries: HTMLString[] = [...doc.querySelectorAll(".word-details-pane")].map(($pane, i) =>
        sanitizeToSafeHtml(`
...
function wrapNoResult(langCode: string): DictSearchResult<HjdictResultRelated> {
    return {
        result: {
            type: "related",
            langCode,
            content: sanitizeToSafeHtml('<p style="text-align:center;">No Result</p>'),
        },
    };
}
